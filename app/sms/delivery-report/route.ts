...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('📨 Received delivery report(s):', JSON.stringify(reports, null, 2))
    ...
    for (const report of reports) {
      try {
        ...
        console.log(`🔍 Looking for message with provider_message_id: ${providerMessageId}`)

        // Map Africa's Talking status to our status
        const mappedStatus = mapAfricasTalkingStatus(status)

        // First, check if message exists with this provider_message_id
        const existingMessage: any = await executeQuery(
          'SELECT id, status FROM sms_messages WHERE provider_message_id = ? LIMIT 1',
          [providerMessageId]
        )

        if (!existingMessage || !Array.isArray(existingMessage) || existingMessage.length === 0) {
          console.warn(`⚠️ Message not found with provider_message_id: ${providerMessageId}`)
          
          // Diagnostic: Show all messages with NULL provider_message_id
          const nullMessages: any = await executeQuery(
            'SELECT id, recipient_phone, status, provider_message_id FROM sms_messages WHERE provider_message_id IS NULL ORDER BY created_at DESC LIMIT 5'
          )
          
          if (nullMessages && Array.isArray(nullMessages) && nullMessages.length > 0) {
            console.warn(`📊 Found ${nullMessages.length} messages with NULL provider_message_id:`, nullMessages)
            console.warn(`💡 This suggests messageId might not be coming from Africa's Talking API response`)
          }
          
          results.push({
            providerMessageId,
            success: false,
            error: 'Message not found in database - possible messageId mismatch',
          })
          continue
        } else {
          console.log(`✅ Found message ID: ${existingMessage[0]?.id}, Current status: ${existingMessage[0]?.status}`)
        }
        ...
        console.log(
          `✅ Delivery report processed: Provider ID ${providerMessageId} -> ${mappedStatus} (${updateResult.affectedRows} rows updated)${
